...
from deval.core.android.constant import (DEFAULT_ADB_PATH, IP_PATTERN, SDK_VERISON_NEW)
from deval.core.error import (AdbError, AdbShellError, DevalError, DeviceConnectionError)
...
from deval.utils.snippet import get_std_encoding, reg_cleanup, split_cmd
...
class ADB(object):
    ...
    def install_app(self, filepath, replace=False):
        ...
        if re.search(r"Failure \[.*?\]", out):
            print(out)
            raise DevalError("Installation Failure")
    ...
    def swipe(self, tuple_x0y0, tuple_x1y1, duration=500):
        ...
        Raises:
            DevalError: if SDK version is not supported
        ...
        if version <= 15:
            raise DevalError('swipe: API <= 15 not supported (version=%d)' % version)
        elif version <= 17:
    ...
    def get_top_activity(self):
        ...
        Raises:
            DevalError: if top activity cannot be obtained
        ...
        if m:
            ...
            raise DevalError("Can not get top activity, output:%s" % dat)
    ...
    def is_screenon(self):
        ...
        Raises:
            DevalError: if screen state can't be detected
        ...
        raise DevalError("Couldn't determine screen ON state")
    ...
    def is_locked(self):
        ...
        Raises:
            DevalError: if lock screen can't be detected
        ...
        if not m:
            raise DevalError("Couldn't determine screen lock state")
        return (m.group(1) == 'true')
    ...
    def path_app(self, package):
        ...
        Raises:
            ...
            DevalError: if package is not found on the device
        ...
        if 'package:' not in output:
            raise DevalError('package not found, output:[%s]' % output)
    ...
    def check_app(self, package):
        ...
        Raises:
            DevalError: if package is not found
        ...
        if package not in output:
            raise DevalError('package "{}" not found'.format(package))
        return True
    ...
    def start_app_timing(self, package, activity):
        ...
        if not re.search(r"Status:\s*ok", out):
            raise DevalError("Starting App: %s/%s Failed!" % (package, activity))
