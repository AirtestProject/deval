...
import re
import six
from deval.core.android.yosemite import Yosemite
from deval.core.android.constant import YOSEMITE_PACKAGE
from deval.core.error import DevalError
...
class Recorder(Yosemite):
    ...
    def start_recording(self, max_time=1800, bit_rate=None, vertical=None):
        ...
        if getattr(self, "recording_proc", None):
            raise DevalError("recording_proc has already started")
    ...
    def stop_recording(self, output="screen.mp4", is_interrupted=False):
        ...
        Raises:
            DevalError: if recording was not started before
        ...
        for line in p.stdout.readlines():
            ...
            if m:
                ...
                return True
        raise DevalError("start_recording first")
