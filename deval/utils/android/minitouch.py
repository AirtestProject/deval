...
from deval.utils.snippet import (get_std_encoding, on_method_ready, ready_method, reg_cleanup)
...
class Minitouch(object):
    ...
    def __init__(self, device, backend=False):
        self.device = device
        self.adb = device.adb
    ...
    def install_and_setup(self):
        ...
        self.size_info = self.device.screen_component.get_display_info()
        self.setup_server()
        if self.backend:
